...
trait GeoOperations[F[+_]] extends GeoApi[F] { self: Redis[F] =>
  ...
  override def geoadd(key: Any, members: Iterable[Product3[Any, Any, Any]]): F[Resp[Option[Int]]] =
    send("GEOADD", key :: flattenProduct3(members))(asInt)

  override def geopos[A](
      key: Any,
      members: Iterable[Any]
  )(implicit format: Format, parse: Parse[A]): F[Resp[Option[List[Option[List[Option[A]]]]]]] =
