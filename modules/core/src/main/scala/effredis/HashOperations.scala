...
trait HashOperations[F[+_]] extends HashApi[F] { self: Redis[F, _] =>
  ...
  override def hset(key: Any, field: Any, value: Any)(implicit format: Format): F[Resp[Boolean]] =
    send("HSET", List(key, field, value))(asBoolean)

  override def hset1(key: Any, field: Any, value: Any)(implicit format: Format): F[Resp[Option[Long]]] =
  ...
  override def hmget[K, V](
      key: Any,
      fields: K*
  )(implicit format: Format, parseV: Parse[V]): F[Resp[Option[Map[K, V]]]] =
  ...
  override def hincrby(key: Any, field: Any, value: Long)(implicit format: Format): F[Resp[Option[Long]]] =
  ...
  override def hdel(key: Any, field: Any, fields: Any*)(implicit format: Format): F[Resp[Option[Long]]] =
    send("HDEL", List(key, field) ::: fields.toList)(asLong)

  override def hlen(key: Any)(implicit format: Format): F[Resp[Option[Long]]] =
    send("HLEN", List(key))(asLong)

  override def hkeys[A](key: Any)(implicit format: Format, parse: Parse[A]): F[Resp[Option[List[A]]]] =
    send("HKEYS", List(key))(asList.map(_.flatten))

  override def hvals[A](key: Any)(implicit format: Format, parse: Parse[A]): F[Resp[Option[List[A]]]] =
  ...
  override def hgetall1[K, V](
    ...
    ev.fmap(fa) {
      case s @ Value(Some(m)) if m.nonEmpty => s
      case _                                => Value(None)
    }
  }
