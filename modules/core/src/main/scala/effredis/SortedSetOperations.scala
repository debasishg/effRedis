...
trait SortedSetOperations[F[_]] extends SortedSetApi[F] { self: Redis =>
  ...
  override def zrem(key: Any, member: Any, members: Any*)(implicit format: Format): F[Option[Long]] =
    send("ZREM", List(key, member) ::: members.toList)(asLong)

  override def zincrby(key: Any, incr: Double, member: Any)(implicit format: Format): F[Option[Double]] =
    send("ZINCRBY", List(key, incr, member))(asBulk(Parse.Implicits.parseDouble))

  override def zcard(key: Any)(implicit format: Format): F[Option[Long]] =
    send("ZCARD", List(key))(asLong)

  override def zscore(key: Any, element: Any)(implicit format: Format): F[Option[Double]] =
  ...
  override def zrank(key: Any, member: Any, reverse: Boolean = false)(implicit format: Format): F[Option[Long]] =
    send(if (reverse) "ZREVRANK" else "ZRANK", List(key, member))(asLong)

  override def zremrangebyrank(key: Any, start: Int = 0, end: Int = -1)(implicit format: Format): F[Option[Long]] =
